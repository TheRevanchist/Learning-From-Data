# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 11:18:40 2016
...
def run_simulation(number_of_points, dimensions): 
    ...
    for i in range(number_of_simulations):
        ...
        weights = perceptron(points, number_of_points, y, weights) # run perceptron
        ...
        svm_data = np.delete(points, 0, 1)
        if len(list(set(y))) == 1:
            continue
        machine.fit(svm_data, y)
        svm_error = 0.0
        
        svm_predict = np.delete(test_points, 0, 1)
        predictions = machine.predict(svm_predict)
        svm_error = len(test_y[predictions != test_y]) / float(number_of_test_points)
        
        if svm_error < test_error:
            svm_win += 1
        support_vectors += np.sum(machine.n_support_)   
            
        
    return float(svm_win)/number_of_simulations, float(support_vectors)/number_of_simulations
...
svm_win_percentage, support_vectors = run_simulation(number_of_points, dimensions)
print "SVM win: " + str(svm_win_percentage)
print "Number of support vectors is: " + str(support_vectors)